define(["fetchHelper", "dom", "registrationServices", "loading", "confirm", "globalize", "emby-linkbutton", "emby-collapse", "emby-input", "emby-button"], function(fetchHelper, dom, registrationServices, loading, confirm, globalize) {
    "use strict";

    function load(page) {
        loading.show(), ApiClient.getPluginSecurityInfo().then(function(info) {
            page.querySelector("#txtSupporterKey").value = info.SupporterKey || "", info.SupporterKey && !info.IsMBSupporter ? (page.querySelector("#txtSupporterKey").classList.add("invalidEntry"), page.querySelector(".notSupporter").classList.remove("hide")) : (page.querySelector("#txtSupporterKey").classList.remove("invalidEntry"), page.querySelector(".notSupporter").classList.add("hide")), info.IsMBSupporter ? getPremiereStatus(info.SupporterKey).then(function(statusInfo) {
                if (statusInfo) {
            ...
            }) : page.querySelector(".isSupporter").classList.add("hide"), loading.hide()
        })
    }
    ...
    function getSubscriptionHtml(subs, key) {
        return subs.map(function(item) {
            var itemHtml = "",
                ...
                openTag = tagName = '<button type="button"',
                cssClass = makeLink ? " lnkSubscription" : "";
    ...
    function loadUserInfo(page) {
        ApiClient.getPluginSecurityInfo().then(function(info) {
            info.IsMBSupporter ? page.querySelector(".supporterContainer").classList.add("hide") : page.querySelector(".supporterContainer").classList.remove("hide")
        })
    }
    ...
    function updateSupporterKey(e) {
        ...
        return ApiClient.updatePluginSecurityInfo(info).then(function() {
            Dashboard.resetPluginSecurityInfo(), loading.hide(), alertText(key ? {
